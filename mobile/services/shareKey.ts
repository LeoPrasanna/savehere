 ...
 *
 * ⚠️ THE KEY AND THE SHAPE ARE A CONTRACT WITH KOTLIN. AsyncStorage on Android
 * is the SQLite file `RKStorage` (table `catalystLocalStorage`), NOT
...
export async function ensureShareKey(): Promise<void> {
  // Android is the only platform with the Activity. iOS needs a Share
  // Extension plus an App Group, both blocked on the Apple Developer account;
  // when that exists this becomes `Platform.OS === 'web'`.
