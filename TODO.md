...
- [x] **Ask-your-library cost reduction** — `librarian.ask_library` now retrieves only the top-15 most relevant saves (term-overlap scoring, title/tags weighted) instead of dumping up to 60 into every prompt — ~3–4x fewer input tokens per ask. The per-user quota runs comfortably at 30/day (`AI_DAILY_LIMIT`). *(Next-level: embeddings-based retrieval for semantic matches.)*
