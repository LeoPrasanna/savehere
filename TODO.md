...
- [x] **Tap-to-watch + honest link-failure popup** — the detail-screen hero thumbnail (now with a "Watch" chip) and the source-URL row open the original post through `mobile/services/openLink.ts`; if it can't open (post deleted/private, no handler app, popup blocked, offline) a popup explains the likely causes instead of failing silently.
...
- [x] **Appearance: accent themes (web)** — ProfilePanel → Appearance now offers 5 accent palettes (Ember default, Iris, Ocean, Forest, Rose; `accentThemes` in `constants/theme.ts`). Because `colors.accent` is frozen into module-level `StyleSheet.create` calls, a pick persists (`@savehere:accent:v1`) and **reloads** the app; theme.ts re-applies it at module init via a sync localStorage read. Verified end-to-end on web (swatch click → reload → fully re-themed, zero stale colors, session intact). ⚠️ **Native**: the pick saves to AsyncStorage but can't be read synchronously at module init — native stays on Ember until a sync store (MMKV) or a theme-context refactor lands; the picker says so honestly instead of half-applying.
