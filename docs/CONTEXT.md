...
- **Deploy-ready (2026-06-23):** backend is prepared for a stable host to end the Codespace-teardown churn — `render.yaml` blueprint, env-driven CORS (`ALLOWED_ORIGINS`) + `DATABASE_URL` (SQLite default, Postgres-ready, SQLite-only connect_args guarded), `$PORT` start command, `/health` check. Steps + caveats in [`docs/DEPLOY.md`](DEPLOY.md). The deploy itself is owner action (connect repo, set `ANTHROPIC_API_KEY`). Reminder: deploying fixes **uptime, not extraction** — Render/Railway are datacenter IPs, so transcripts still need the proxy work above; oEmbed + `facebookexternalhit` captions keep working.
